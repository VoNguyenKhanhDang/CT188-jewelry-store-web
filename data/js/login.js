...
window.frmValidate5 = function (frm) {
  ...
  const taiKhoanHopLe = danhSachHienTai.find(
    (user) => user.email.toLowerCase() === emailDangNhap,
  );
  ...
  if (!taiKhoanHopLe) {
    emailInp.parentElement.classList.add("error");
    emailErr.textContent =
      "Tài khoản Email này chưa được đăng ký trên hệ thống.";
